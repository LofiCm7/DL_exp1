...
def diabetes_data(test_size, random_state, valid_size, rescale_test=False): # 划分数据集并提取数据
    # 加载糖尿病数据集
    ...
    ### 提取训练集0的标签y的平均值与标准差用于标准化训练集0以及后续恢复原始标签范围 ###
    mean = y_train_0.mean()
    std = y_train_0.std()
    ### 对训练集0的标签进行标准化 ###
    y_train_0 = normalize_labels(y_train_0, mean, std)
    ### 可选：对测试集的标签进行标准化 ###
    if rescale_test:
...
def load_data(test_size, random_state, valid_size, batch_size, rescale_test=False):
    X_train, X_valid, X_test, y_train, y_valid, y_test, diabetes, mean, std = diabetes_data(test_size, random_state, valid_size, rescale_test)
...
if __name__ == "__main__":
    ...
    load_data(test_size=0.2, random_state=42, valid_size=0.2, batch_size=32, rescale_test=False)